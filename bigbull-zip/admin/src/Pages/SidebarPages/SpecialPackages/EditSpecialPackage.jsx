...
import { _updatePackage, createPackage } from '../../../redux/actions/packageActions'
...
const EditSpecialPackage = () => {
    ...
    const { updateLoading } = useSelector(state => state.package)
    ...
    useEffect(() => {
        const fetchSinglePackages = async () => {
            try {
                ...
                setData(prev => ({...doc}));
                setTags(doc?.depositRange);
                setImage(`${baseURL}/packages/${doc?.image}`)
                setLoading(false);
            } catch (error) {
                setLoading(false);
                toastError(error);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const packageData = {...data , depositRange : tags };
        delete packageData.image;
        if(isBase64(image)){
            packageData.image = image;
        }
        dispatch(_updatePackage(packageId , packageData , navigate , 2));
    }


    return (
        <div className='p-4'>
            <h1 className='text-lg font-semibold mb-4'>Edit package</h1>
            <div className='bg-black border rounded-xl shadow-lg sm:p-5 px-4'>
...
export default EditSpecialPackage
